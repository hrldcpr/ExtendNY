...
function findLatLng(pos) {
  ...
  return gspherical.computeOffset(
    ave0,
    getStreetRadius(pos.street) - circumference / 4,
    phi,
  );
}
...
function findIntersection(pos) {
  ...
  return {
    street: Math.round(street) % nStreets,
...
$(function () {
  ...
  if (navigator.geolocation) {
    ...
    locationDiv.show();
    map.controls[gmaps.ControlPosition.TOP_LEFT].push(
      $("#location-control")[0],
    );
  }
  ...
  function moveUserSign(latLng, noHash) {
    if (!userSign) userSign = new SignOverlay($("#user").show(), latLng, map);
    userSign.setLatLng(latLng);
    if (!noHash) updateHash(findIntersection(latLng), map.getZoom());
  }
  ...
  function showGrid() {
    ...
    for (var type in grid) {
      ...
      for (var i in roads) roads[i].clip = true;
      ...
      for (var k = -20; k < 20; k++) {
        ...
        if (i in roads)
          ...
          var extra = { map: map };
          if (i == 0) $.extend(extra, { strokeColor: streetYellow, zIndex: 5 });
  ...
  function mouseMoved(e) {
    ...
    mouseTimer = setTimeout(function () {
      ...
      if (mouseRoadsTimer) clearTimeout(mouseRoadsTimer);
